...
const LatestActivity = ({ setView }: LatestActivityProps) => {
    ...
    }[type] || { icon: 'fa-cog', color: 'text-slate-500' });
                    ...
                    {latestActivities.map((activity) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div>
                                    <p className={`text-sm ${isClickable ? 'group-hover:text-cnk-accent-primary' : ''}`}>{t(activity.messageKey, activity.replacements)}</p>
                                    <p className="text-xs text-cnk-txt-muted-light">{new Date(activity.timestamp).toLocaleString()}</p>
                                </div>
                            </li>
                        );
                    })}
                </ul>
            ) : <p className="text-sm text-center mt-8">{t('noRecentActivity')}</p>}
        </div>
    );
};
